...
OPTION_s = {}
OPTION_s["FEIG"] = {}
OPTION_s["FEIG-S"] = [{"use_hybrid": True}, {"use_hybrid": True}]
OPTION_s["FEIG-R1"] = {}
OPTION_s["FEIG-R2"] = {}
OPTION_s["FEIG-R3"] = {}
OPTION_s["FEIG-R4"] = {}
...
def send_big_target_warning(target, fn):
    ...
    cmd.append("-s")
    cmd.append(
        "A big CASP14 target has submitted: %s (%d)"
        % (target["target_id"], len(target["sequence"]))
    )
    cmd.append("-r")
    cmd.append(PARAM_MY_EMAIL)
    cmd.append(PARAM_MY_EMAIL)
...
def get_tarball(target_s):
    ...
    for target in target_s:
        ...
        for tarball_days, tarball_type in zip([5, 7, 9], ["stage1.3D", "stage2.3D", "3D"]):
            ...
            sp.call(
                ["wget", "-q", "-c", url, "-O", "%s/%s" % (run_home, tgz_fn)], stderr=sp.DEVNULL
            )
