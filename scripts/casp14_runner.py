...
def initialize_TR_server(target, use_hybrid=False, use_extensive=False):
    url = "http://predictioncenter.org/download_area/CASP14/extra_experiments/%s.pdb.txt" % (
        target["target_id"]
    )
...
def initialize_human_target(
    target_id, meta, pdb_fn, use_hybrid=False, use_extensive=False, human=None
):
    EXEC = "%s/bin/casp14_refine.py" % PIPE_HOME
    #
    cmd = [EXEC]
...
def run_human(target_s):
    for target in target_s:
            ...
            if target[status_name] is None:
                ...
                if META_s[meta] == "RaptorX_TS1":
                    ...
                    if os.path.exists(pdb_fn):
                        pdb_fn_s = glob.glob(
                            "%s/%s.raptorX/model_1d*.pdb" % (TARBALL_HOME, target["target_id"])
                        )
                        if len(pdb_fn_s) > 0:
                ...
                if os.path.exists(pdb_fn):
                    if not multiple_domain:
                        initialize_human_target(
                            target["target_id"], meta, pdb_fn, human=target["human"]
                        )
                        ...
                        for i, pdb_fn in enumerate(pdb_fn_s):
                            ...
                            initialize_human_target(
                                target_domain_id, meta, pdb_fn, human=target["human"]
                            )
        ...
        if target["status_H"] is None:
            human_status = [
                target["status%s" % meta] == "DONE" for meta in ["", "_R1", "_R2", "_R3"]
            ]
