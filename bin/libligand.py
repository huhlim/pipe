...
def set_bsite(info, top_fn, ligand_pdb_fn):
    ...
    dist_s = mdtraj.compute_distances(ligand_pdb, atomPair_s)[0].reshape(
        (len(proteinIndex), len(ligandIndex))
    )
    #
    ...
    for i, align in enumerate(bsiteAligned):
        ...
        for i_prot, i_lig, d in geom:
            ...
            info["bsite_geometry"].append(
                (protResidueIndex, protAtmName, ligResidueIndex, ligAtmName, float(d))
            )
    #
    ...
    for residue in ligand.top.residues:
        ...
        for atmName, placed in zip(atom_s, status):
            ...
            if not atmName.startswith("H"):
                sys.exit(
                    "ERROR: missing heavy atom (%s) in the input ligand (%s).\n"
                    % (atmName, resName)
                )
...
def get_ligand_restratint(pdb, psf_fn, info):
    ...
    for geom in info["bsite_geometry"]:
        ...
        #
        pair = [
            (residue.segment_id, residue.resSeq, atmName),
            (lig.segment_id, lig.resSeq, ligAtmName),
            d,
        ]
