...
def get_prod_info(run_home):
    ...
    iter_simul_time = dynsteps * dyntstep / 1000.0
    ...
    for method in METHODs:
        ...
        if method == "prod":
            ...
            for _, task in task_s:
                ...
                if speed > 0.0:
                    t_left = (t_target - t_curr) / speed * 86400.0
                ...
                if t_left is not None and status == "RUN":
                    wrt.append("%6.2f hrs_left" % (t_left / 3600.0))
                    wrt.append(t_est)
...
def remove_from_slaves(json_job_s):
    ...
    for host in task_by_host:
        ...
        with host_json.open("wt") as fout:
            fout.write(json.dumps({"resource": host_info["resource"], "task": task_s}, indent=2))
...
def reallocate_task(json_job, method, index, resource):
    ...
    if task["resource"][1] is not None:
        ...
        if host_json.status():
            ...
            with host_json.open("wt") as fout:
                fout.write(
                    json.dumps({"resource": host_info["resource"], "task": task_s}, indent=2)
                )
    #
