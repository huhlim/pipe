...
def main():
    ...
    import_module("equil").prep(job, 0, [out[0] for out in locPREFMD_out], \
                                            path.Path("%s/equil_casp14.json"%(DEFAULT_HOME)))
    if not run(job, arg.wait_after_run):
        return 
    
    # prod
    n_traj = 5
    prod_input = path.Path("%s/prod_casp14.json"%DEFAULT_HOME)
    ...
    # average
    import_module("average").prep(job, '%s.casp12.prod_0'%job.title,  [0], path.Path("%s/average.json"%DEFAULT_HOME), rule='casp12')
    import_module("average").prep(job, '%s.score.prod_0'%job.title,   [0], path.Path("%s/average.json"%DEFAULT_HOME), rule='score')
    import_module("average").prep(job, '%s.cluster.prod_0'%job.title, [0], path.Path("%s/average.json"%DEFAULT_HOME), rule='cluster')
    if n_init > 1:
        prodIndex = [i for i in range(n_init)]
        import_module("average").prep(job, '%s.casp12'%job.title, prodIndex, path.Path("%s/average.json"%DEFAULT_HOME), rule='casp12')
        for i in range(1, n_init):
            import_module("average").prep(job, '%s.score.prod_%d'%(job.title, i), [i], path.Path("%s/average.json"%DEFAULT_HOME), rule='score')
        import_module("average").prep(job, '%s.score'%job.title, prodIndex, path.Path("%s/average.json"%DEFAULT_HOME), rule='score')
        import_module("average").prep(job, '%s.cluster'%job.title, prodIndex, path.Path("%s/average.json"%DEFAULT_HOME), rule='cluster')
    if not run(job, arg.wait_after_run):
        return
    #
    average_out = []
