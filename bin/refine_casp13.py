...
def main():
    ...
    import_module("equil").prep(
        ...
        path.Path("%s/equil_casp13.json" % (DEFAULT_HOME)),
    )
    if not run(job, arg.wait_after_run):
        return
    ...
    # prod
    n_traj = 5
    prod_input = path.Path("%s/prod_casp13.json" % DEFAULT_HOME)
    ...
    import_module("average").prep(
        job, "%s.score" % job.title, [0], path.Path("%s/average.json" % DEFAULT_HOME), rule="score"
    )
    import_module("average").prep(
        job,
        "%s.cluster" % job.title,
