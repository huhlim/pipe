...
def _calc_distance(X, Y, pbc, dist_bin):
    dr = np.abs(Y - X)
...
def calc_distance(cntr_i, cntr_j, pbc, dist_bin, is_same):
    pair_s = []
    for i,c_i in enumerate(cntr_i):
        for j,c_j in enumerate(cntr_j):
            if is_same and (j <= i):
                continue
            elif (not is_same) and (j < i):
                continue
            pair_s.append((i,j))
    if len(pair_s) == 0:
        return None
    ...
    for (i,j) in pair_s:
        distr += _calc_distance(cntr_i[i], cntr_j[j], pbc, dist_bin)
    return distr

def run(molecule_s, top, dcd_fn_s, T_STRIDE, T_SKIP, D_GRID, \
    ...
    distr_s = {}
    distr_s['dist_bin'] = dist_bin * 10.    # angstrom
    ...
    for i,mol_i in enumerate(cntr_s):
        for j,mol_j in enumerate(cntr_s):
            ...
            d = calc_distance(cntr_s[mol_i], cntr_s[mol_j], pbc, dist_bin, (i==j))
            if d is not None:
                distr_s[mol_pair] = d
    return distr_s
...
def main():
    ...
    for traj_fn in arg.traj_s:
        try:
            ...
            dcd_fn_s = [traj_fn]
        #
        distr = run(molecule_s, top, dcd_fn_s, T_STRIDE, T_SKIP, D_GRID, 
