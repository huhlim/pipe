...
from libpdb import PDB, Sequence
import libsalign
...
class HHresult:
    ...
    def write_in_PIR(self, query_name="Query"):
        ...
        if self.t_res is None:
            ...
            wrt.append(
                "structure:%s:%4d:%s:%4d:%s::::\n"
                % (self.pdb_id, self.t_res[0], self.chain_id, self.t_res[1], self.chain_id)
            )
        wrt.append("%s*\n" % self.t_align)
        return "".join(wrt)
    ...
    def pdb_fn(self, pdb70_db=None):
        ...
        if pdb70_db is not None:
            out = sp.check_output(
                "ffindex_get %s %s %s 2>/dev/null"
                % ("%s_pdb.ffdata" % pdb70_db, "%s_pdb.ffindex" % pdb70_db, self.pdb_id),
                shell=True,
            )
            self._pdb_fn0 = Path("%s.pdb" % self.pdb_id)
        ...
        if os.path.exists("%s/pdb%s.ent" % (PDB_HOME, self.pdb_id[:4])):
            ...
            sp.call("pdb_get -f %s &> /dev/null" % self.pdb_id, shell=True)
            self._pdb_fn0 = Path("%s.pdb" % self.pdb_id)
        #
        if self._pdb_fn0.status(size=1024):
            return self._pdb_fn0
    ...
    def prepare_pdb(self, pdb70_db=None):
        ...
        if self.chain_id not in chainID_s:
            if self.is_pdb70:
                sys.stderr.write(
                    "Error: failed to find the template chain in the PDB file, %s\n" % self.pdb_fn()
                )
                self.valid = False
                return
            else:
        ...
        #
        aligned_resNo = resNo[
            align1.align.index(aligned_sequence[0]) : align1.align.rindex(aligned_sequence[-1]) + 1
        ]
        k = 0
        t_align = []
        self.t_res = (9999, -9999)
        ...
        for i in range(len(self.t_align)):
            if self.t_align[i] == "-" or k == len(aligned_resNo):
                ...
                self.t_res = (
                    min(aligned_resNo[k], self.t_res[0]),
    ...
    with open(hhr_fn) as fp:
        ...
        for line in fp:
            if line.startswith("Query "):
                ...
                if (
                    (hh_id[:4].lower() in exclude_s)
                    or (hh_id[:4].upper() in exclude_s)
                    or (hh_id in exclude_s)
                    or (pdb_id in exclude_s)
                ):
                    exclude = True
                    continue
                exclude = False
