...
def check_clash(X, Y, BOX_WIDTH, cutoff):
    if len(Y) == 0:
        return False
    #
    rX = np.array(X).astype(np.float16) ; nX = rX.shape[0]
    rY = np.concatenate(Y).astype(np.float16) ; nY = rY.shape[0]
    #
...
def place(molecule_s, n_place, box_width):
    ...
    for molecule in molecule_s:
        ...
        #
        xyz -= cntr ; xyz_s.append(xyz)
        xyz_hv -= cntr ; xyz_hv_s.append(xyz_hv)
    #
    ...
    N_FAIL = 0
    while True:
        clash_cutoff = PARAM_CLASH_CUTOFF - 0.01 * int(N_FAIL/100.)
...
def convert_to_pdb(molecule_s, n_place, xyz, box_width):
    ...
    out = mdtraj.Trajectory(xyz, top, \
            unitcell_lengths=box_width*np.ones(3),\
...
def calc_system_size(arg):
    ...
    if np.all(n_place > 0):
        ...
        if np.any(conc[n_place > 0] > 0.):
            ...
            elif np.where(conc <= 0.)[0].shape[0] == 1 and conc_total > 0.:
                if arg.unit == 'mM':
                    c = conc[conc > 0.].sum()
                    conc[conc <= 0.] = (conc_total - c)
                else:
                    c = (conc[conc > 0.] * 1e-3) * mass
                    c = conc_total - c.sum()
                    conc[conc <= 0.] = c / mass[conc <= 0.] * 1e3
