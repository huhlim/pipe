    ...
    def apply_map(self, prob, verbose=False):
        def get_domain(self, Xs, segment_i, verbose=False):
            ...
            for xi in Xs:
                ...
                if not verbose:
                    continue
                domain_residue = [
                    (1 + np.where(self.segment_index == segment_i[l])[0]).tolist()
                    for l in sorted(xi)
                ]
                domain = []
        ...
        for i in range(n_graphs):
            ...
            for X in community:
                ...
                domain_size = [
                    min((np.where(d == True)[0].shape[0]), np.where(d != True)[0].shape[0])
                    for d in domain
                ]
                if min(domain_size) < PARAM_DOMAIN_MIN_RES:
                    break
...
def get_soft_domain_boundary(mask0, soft=PARAM_DOMAIN_BOUNDARY * 2):
    ...
    #
    weights = np.arange(1, 1 + soft, dtype=float) / (1.0 + soft)
    #
