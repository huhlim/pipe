...
def get_hist(n_residue, feature_s):
    ...
    for feature_name in feature_s:
        hist[feature_name] = np.zeros(
            (n_residue, n_residue, BINS[feature_name].shape[0]), dtype=np.float32
        )
        #
        for i in range(n_residue):
            for j in range(n_residue):
                ...
                if feature_name in ["dist", "omega"] and j < i:
                    ...
                    continue
                #
                h = np.histogram(feature_s[feature_name][:, i, j], bins=BINS[feature_name])[
                    0
                ].astype(np.float32)
                h /= h.sum()
...
def vonMises(m, kappa, X0):
    return np.exp(kappa * np.cos(X0[None, :] - m[:, None])) / (2.0 * np.pi * np.i0(kappa))


def get_distr(n_residue, feature_s, mask):
    TYPE = {"dist": "distance", "phi": "angle", "omega": "dihedral", "theta": "dihedral"}
    SIGMA = {"distance": 0.5, "angle": 20.0, "dihedral": 20.0}
    SPACE = {"distance": 0.5, "angle": np.deg2rad(15.0), "dihedral": np.deg2rad(15.0)}
    #
    DIST_BINS = np.linspace(2.0, 20.0, 37) + SPACE["distance"] * 0.5
    ANGLE_BINS = np.deg2rad(np.linspace(0.0, 180.0, 13)) + SPACE["angle"] * 0.5
    DIHEDRAL_BINS = np.deg2rad(np.linspace(-180.0, 180.0, 25)) + SPACE["dihedral"] * 0.5
