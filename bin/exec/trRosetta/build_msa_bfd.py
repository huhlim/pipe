...
OPTIONs['trRosetta']['HHblits_bfd'] = "/feig/s1/huhlim/db/hhsuite/bfd/bfd"
...
def run_trRosetta_HHblits_bfd(title, in_msa, n_proc=PARAM_N_PROC):
    out_msa = path.Path('%s.hhblits.bfd.a3m'%(title))
    #
    cmd = []
    cmd.append("hhblits")
    cmd.extend(["-i", in_msa.short()])
    ...
    cmd.extend(["-o", "/dev/null"])
    cmd.extend(["-cpu", "%d"%n_proc])
    cmd.extend(["-d", OPTIONs['trRosetta']['HHblits_bfd']])
    cmd.extend(OPTIONs['trRosetta']['HHblits'])
    if not out_msa.status():
        system(cmd)
    if not out_msa.status():
...
def run_trRosetta_search(title, fa_fn, n_proc=PARAM_N_PROC):
    ...
    if final_msa.status():
        return final_msa
    #
    has_enough_sequence, out_hhblits_msa = run_trRosetta_iterative_HHblits_search(title, fa_fn, n_proc=n_proc)
    if has_enough_sequence:
        ...
        bfd_input_msa = path.Path('%s.bfd_input.a3m'%title)
        if not bfd_input_msa.status():
            with bfd_input_msa.open("wt") as fout:
                with out_hhblits_msa.open() as fp:
                    fout.write(fp.read())
                if out_hmmer_msa is not None:
                    with out_hmmer_msa.open() as fp:
                        fout.write(fp.read())
        merged_msa = run_trRosetta_HHblits_bfd(title, bfd_input_msa, n_proc=n_proc)
    #
