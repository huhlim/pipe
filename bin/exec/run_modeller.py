...
def parse_pir(pir_fn):
    ...
    if len(query) != 1:
        if len(query) == 0:
            ...
            sys.stderr.write(
                "ERROR: there are multiple (%d) QUERY sequences in the PIR file\n" % len(query)
            )
        sys.exit()
...
def main():
    ...
    arg.add_argument(
        dest="pir_fn", metavar="ALIGNMENT", help="Sequence alignment file in PIR format"
    )
    arg.add_argument(
        "-p",
        "--pdb",
    ...
    arg.add_argument(
        "-i", "--initial", dest="ini", metavar="INI", help="Initial model for structure building"
    )
    arg.add_argument(
        "-r", "--restraint", dest="rsr", metavar="RSR", help="Restraint file for structure building"
    )
