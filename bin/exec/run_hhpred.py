...
from libhhsuite import read_a3m, parse_hhr, read_sequence
...
def run_hhblits(fa_fn, db, prefix, n_proc, verbose):
    ...
    system(cmd, verbose=verbose, stdout=True, errfile="/dev/null")
    return a3m_fn
...
def build_model(out_prefix, glo_fn, fa_fn, pdb70_db, n_proc, allow_fix_build, verbose, alt=0):
    ...
    if os.path.exists("%s_pdb.ffdata" % pdb70_db) and os.path.exists("%s_pdb.ffindex" % pdb70_db):
    ...
    #
    cmd = []
    cmd.append("%s/run_modeller.py" % EXEC_HOME)
...
def get_A3M(templ_id, pdb70_db, verbose):
    ...
    try:
        with open("%s.a3m" % templ_id, "wt") as fout:
            system(cmd, verbose=verbose, outfile=fout, stdout=True, errfile="/dev/null")
        if os.path.exists(a3m_fn):
            return a3m_fn
    except:
        return None
...
def main():
    arg = argparse.ArgumentParser(prog="run_hhpred")
    arg.add_argument(dest="fa_fn", metavar="SEQ", help="Sequence file in FASTA format")
    arg.add_argument(
        "-d",
        "--database",
        dest="db",
    ...
    arg.add_argument(
        "--evalue",
        dest="Evalue_cutoff",
        help="E-value cutoff for template selection (default=1e-3)",
        default=0.001,
    )
    arg.add_argument(
        "--seq_id",
        dest="SeqID_cutoff",
        help="SeqID cutoff for template selection (default=20%%)",
        default=20.0,
    )
    arg.add_argument(
        "--pdb", dest="pdb_fn0", default=None, help="The template PDB file for inverse.build mode"
    )
    arg.add_argument(
        "--n_model",
        dest="n_model",
    ...
    arg.add_argument("--include", dest="include", default=None, help="Template list file")
    arg.add_argument("--exclude", dest="exclude", default=None, help="Template exclusion list file")
    arg.add_argument(
        "--force", dest="forced_build", default=False, help="Forced building", action="store_true"
    )
    arg.add_argument(
        "--allow_fix_build",
        dest="allow_fix_build",
        default=False,
        help="Use fixed build mode",
        action="store_true",
    )
    arg.add_argument(
        "-v",
    ...
    if arg.protocol == "":
        out_fn = run_hhsearch(
            a3m_fn, arg.hhdb, arg.prefix, arg.use_global, arg.use_viterbi, arg.n_proc, arg.verbose
        )
        ...
        if arg.include is None:
            templ_id_s = select_template(
                vit_loc_s, Evalue_cutoff=arg.Evalue_cutoff, SeqID_cutoff=arg.SeqID_cutoff
            )[: arg.n_templ]
        else:
