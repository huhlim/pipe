...
def select_water(pdblines, bfactor_cutoff=-1):
    ...
    #
    for w in wat_s:
        if bfactor_cutoff < 0. or w[0] < bfactor_cutoff:
            wrt.extend(w[1])
    #
...
def place_water(r_o, xyz, water_cutoff):
    ...
    r_h1 = r_h1[min_index] ; v_h1 = WATER_ORIENT[min_index]
    #
    rv = np.random.randn(3) ; rv /= np.linalg.norm(rv)
    nv = np.cross(v_h1, rv) ; nv /= np.linalg.norm(nv)
    q0 = Quaternion.from_axis_and_angle(nv, WATER_GEOM[1])
    v_h2 = v_h1.dot(q0.rotate().T)
    r_h2 = []
    for angle in np.linspace(0., 2.0*np.pi, N_SAMPLE):
...
def append_water(in_pdb, water, water_cutoff=0.2):
    ...
    #
    ia = 0
    chain = top.add_chain() ; resNo = 0 ; water_index = 0 ; segName = 'WAT%1d'%water_index
    r_water = []
    for r,water in zip(water.xyz[0], water.topology.residues):
        min_d = calc_min_d(xyz, r)
        if min_d < water_cutoff: continue
        if min_d > WATER_MAX_DISTANCE: continue
        #
...
def main():
    ...
    arg.add_argument('-b', '--bfactor', dest='bfactor_cutoff', default=-1., type=float)
    arg.add_argument('-c', '--cutoff', dest='water_cutoff', default=1., type=float)
    #
    if len(sys.argv) == 1:
