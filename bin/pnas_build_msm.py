...
def main():
    ...
    if not job.has("is_membrane_protein"):
        import_module("equil").prep(
            job, 0, runner_s, path.Path("%s/equil_pnas_msm.json" % (DEFAULT_HOME))
        )
    else:
    ...
    #
    # average
    import_module("average_d").prep_from_msm(
        job, "%s.msm" % job.title, job.msm_fn, path.Path("%s/average.json" % DEFAULT_HOME)
    )
    if not run(job, arg.wait_after_run):
        return
    average_out = []
